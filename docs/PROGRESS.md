...
- **不支持窗口拖动/调整大小/关闭按钮**：当前版本只支持点击置顶。窗口拖动和关闭是
  GUI 路线图规划的后续阶段（阶段 13d+），不是 bug。
- **鼠标光标无运动回调**：当前 `mouse_click_callback` 只在按钮状态变化时触发
  （press/release），无 motion 事件（Decision-008）。这限制了按钮的视觉反馈
  （光标移出按钮不恢复 idle 色）。
